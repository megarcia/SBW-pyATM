...
def main(simdate, exp_name):
    """Get and plot egg counts at oviposition locations."""
    path = '%s_collected' % exp_name
    infnames = sorted(glob('%s/egg_deposition/egg_deposition_*.csv' % path))
    ...
    #
    sim = Simulation()
    print('plotting egg deposition by location')
    ...
    plt.tight_layout()
    title = '%s SBW natal site egg deposition (count)' % simdate
    plt.title(title, fontsize=12)
    plot_fname = '%s/%s_%s_egg_deposition_map.png' % (path, simdate, exp_name)
